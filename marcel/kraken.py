...
from marcel.loaders.ensemble import EnsembleDataset
...
class Kraken(EnsembleDataset):
    ...
    def process(self):
        ...
        for ligand_id in tqdm(ligand_ids):
            ...
            y = torch.tensor([boltz_avg_properties[descriptor] for descriptor in descriptors])
            data_list.append(
                Data(x=data.x, z=data.x[:, 0], edge_index=data.edge_index, edge_attr=data.edge_attr, pos=pos, y=y)
            )
